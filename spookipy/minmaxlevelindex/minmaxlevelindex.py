...
class MinMaxLevelIndex(Plugin):
    ...
    def __init__(
        ...
        if self.nomvar is None:
            ...
            if len(nomvar_list) == 1:
                ...
                raise MinMaxLevelIndexError(
                    "Too many input fields, provide one field or set nomvar : {}".format(nomvar_list)
                )
        ...
        self.plugin_result_specifications = {"ALL": {"label": "MMLVLI", "unit": "1"}}

        self.validate_params_and_input()

    def validate_params_and_input(self):
        validate_nomvar(self.nomvar, "MinMaxLevelIndex", MinMaxLevelIndexError)

        validate_nomvar(self.nomvar_min_idx, "MinMaxLevelIndex", MinMaxLevelIndexError)

        validate_nomvar(self.nomvar_max_idx, "MinMaxLevelIndex", MinMaxLevelIndexError)

        validate_nomvar(self.nomvar_min_val, "MinMaxLevelIndex", MinMaxLevelIndexError)

        validate_nomvar(self.nomvar_max_val, "MinMaxLevelIndex", MinMaxLevelIndexError)
        ...
        if self.bounded:
            if (
                (self.no_meta_df.loc[self.no_meta_df.nomvar == "KBAS"]).empty
                or (self.no_meta_df.loc[self.no_meta_df.nomvar == "KTOP"]).empty
            ):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for dependencies_df, option in self.dependencies_list:
            ...
            min_idx_df = create_result_container(
                var_df, borne_inf, borne_sup, self.nomvar_min_idx, self.bounded, self.plugin_result_specifications
            )
            max_idx_df = create_result_container(
                var_df, borne_inf, borne_sup, self.nomvar_max_idx, self.bounded, self.plugin_result_specifications
            )
            min_val_df = create_result_container(
                var_df, borne_inf, borne_sup, self.nomvar_min_val, self.bounded, self.plugin_result_specifications
            )
            max_val_df = create_result_container(
                var_df, borne_inf, borne_sup, self.nomvar_max_val, self.bounded, self.plugin_result_specifications
            )
        ...
        return self.final_results(df_list, MinMaxLevelIndexError, copy_input=self.copy_input, reduce_df=self.reduce_df)
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            ...
            help="Finds either the maximum or minimum value index or both",
        )
        parser.add_argument(
            "--direction",
            type=str,
            ...
            help="The level iteration direction (upward or downward)",
        )
        parser.add_argument(
            "--bounded",
            dest="bounded",
            action="store_true",
            default=False,
            help="Searches in part of the column (requires fields KBAS and KTOP as inputs) Default: searches the whole column",
        )
        ...
        parser.add_argument(
            ...
            default="KMIN",
            dest="nomvar_min",
            help="Option to change the name of output field KMIN",
        )
        parser.add_argument(
            "--outputFieldName2",
            type=str,
            default="KMAX",
            dest="nomvar_max",
            help="Option to change the name of output field KMAX",
        )

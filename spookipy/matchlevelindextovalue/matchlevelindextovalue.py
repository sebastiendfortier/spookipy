...
class MatchLevelIndexToValue(Plugin):
    ...
    def __init__(
        ...
        self.plugin_result_specifications = {"ALL": {"label": "MLIVAL", "level": 0, "ip1_kind": 2}}
    ...
    def validate_params_and_input(self):
        ...
        self.groups = self.no_meta_df.groupby(by=["grid", "datev", "dateo"])

        for _, group_df in self.groups:
            # print(f'group_df: \n {group_df.ip2.unique()} \n\n')
            ind_df = group_df.loc[group_df.nomvar == self.nomvar_index].reset_index(drop=True)
            if ind_df.empty:
                logging.warning(f"Cannot find {self.nomvar_index} field in this group - skipping the group ")
                continue
            ...
            if (self.nomvar_out is not None) and (len(nomvars) > 1):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, group_df in self.groups:
            ...
            for nomvar in nomvars:
                ...
                var_df = pd.safe_concat([var_df, error_row_df])
        ...
        if len(df_list):
            return self.final_results(
                df_list, MatchLevelIndexToValueError, copy_input=self.copy_input, reduce_df=self.reduce_df
            )
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            ...
            default="IND",
            dest="nomvar_index",
            help="Option to use a different field name other than IND for the field of indices.",
        )
        parser.add_argument(
            "--outputFieldName",
            type=str,
            dest="nomvar_out",
            help="Option to give the output field a different name from the input field name applicable only with one input meteorological field.",
        )
...
def create_result_container(df, b_inf, b_sup, ip1_kind):
    ...
    ip1_enc = RmnInterface.convert_ip(RmnInterface.CONVIP_ENCODE, ip1, kind)
    ip3_enc = RmnInterface.convert_ip(RmnInterface.CONVIP_ENCODE, ip3, kind)

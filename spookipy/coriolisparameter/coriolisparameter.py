...
from ..utils import create_empty_result, existing_results, get_existing_result, initializer
...
class CoriolisParameter(Plugin):
    ...
    @initializer
    def __init__(self, df: pd.DataFrame, copy_input=False, reduce_df=True):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, current_group in self.groups:
            latlon_df = fstpy.get_2d_lat_lon_df(pd.safe_concat([current_group, self.meta_df]))
            ...
            corp_df = create_empty_result(current_group, self.plugin_result_specifications["CORP"])
        ...
        return self.final_results(df_list, CoriolisParameterError, copy_input=self.copy_input, reduce_df=self.reduce_df)

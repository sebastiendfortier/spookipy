...
from ..interpolationutils import (
    find_index_of_lat_lon_not_in_grid,
...
class InterpolationHorizontalPoint(Plugin):
    ...
    def define_output_grid(self):
        if ("LON" in self.lat_lon_df.nomvar.to_list()) and ("LAT" in self.lat_lon_df.nomvar.to_list()):
            ...
            (ni, nj, _, ax, ay, ig1, ig2, ig3, ig4) = fstpy.get_grid_parameters_from_latlon_fields(self.lat_lon_df)
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, current_group in self.groups:
            ...
            others_df = current_group.loc[
                ~current_group.nomvar.isin(["UU", "VV", "PT", "LAT", "LON", ">>", "^^", "^>", "!!", "HY"])
        ...
        other_res_df = fstpy.set_new_grid_identifiers(
            res_df,
            "Y",
            len(self.lat),
            1,
        ...
        if not toctoc_res_df.empty:
            other_res_df = pd.safe_concat([other_res_df, toctoc_res_df])

        if not no_mod_df.empty:
            other_res_df = pd.safe_concat([other_res_df, no_mod_df])

        other_res_df = pd.safe_concat([other_res_df, self.lat_lon_df])
        ...
        self.meta_df = other_res_df.loc[
            other_res_df.nomvar.isin(["^^", ">>", "^>", "!!", "!!SF", "HY", "PT"])
        ].reset_index(drop=True)
        # Separation des donnees des metadonnees avant l'appel a final_results
        res_no_meta_df = other_res_df.loc[
            ~other_res_df.nomvar.isin(["^^", ">>", "^>", "!!", "!!SF", "HY"])
        ].reset_index(drop=True)

        return self.final_results(
            [res_no_meta_df], InterpolationHorizontalPointError, copy_input=False, reduce_df=self.reduce_df
        )

...
class HumidityRelative(Plugin):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        if self.rpn:
            dependencies_list = get_dependencies(self.groups,self.meta_df,'HumidityRelative',self.plugin_mandatory_dependencies_rpn,self.plugin_params,intersect_levels=True)
        else:
            dependencies_list = get_dependencies(self.groups,self.meta_df,'HumidityRelative',self.plugin_mandatory_dependencies,self.plugin_params,intersect_levels=True)
        ...
        for dependencies_df,option in dependencies_list:
            if self.rpn:
                if option==0:
                    ...
                    self.rpn_humidityrelative_from_tt_hu_px(dependencies_df, hu_df, option)
                    ...
                    self.rpn_humidityrelative_from_tt_es_px(dependencies_df, es_df, option)
                else:
                    # dependencies_df = get_intersecting_levels(dependencies_df,self.plugin_mandatory_dependencies_rpn[option])
    ...
    def compute_hu(self, dependencies_df):
        ...
        hu_df = HumiditySpecific(pd.concat([dependencies_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase, temp_phase_switch=self.temp_phase_switch, rpn=True).compute()
    ...
    def compute_es(self, dependencies_df):
        ...
        es_df = DewPointDepression(pd.concat([dependencies_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase, temp_phase_switch=self.temp_phase_switch, rpn=True).compute()
    ...
    def humidityrelative_from_svp_vppr(self, dependencies_df, option):
        ...
        vppr_df = VapourPressure(pd.concat([dependencies_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=(self.temp_phase_switch if self.ice_water_phase!='water' else None)).compute()

...
def test_3(plugin_test_path):
    ...
    with pytest.raises(spookipy.AddElementsByPointError):
        # compute AddElementsByPoint
        _ = spookipy.AddElementsByPoint(src_df0, nomvar_out="TROPLONG").compute()
...
def test_7(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    res_df = spookipy.AddElementsByPoint(masked_df, nomvar_out="HP01", reduce_df=True).compute()
    ...
    df = pd.safe_concat([res_df, src_df0, meta_df])
...
def test_7a(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # # compute AddElementsByPoint
    res_df = spookipy.AddElementsByPoint(src_df0, nomvar_out="HP01", copy_input=True).compute()
...
def test_8(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    with pytest.raises(spookipy.AddElementsByPointError):
        # compute AddElementsByPoint
        _ = spookipy.AddElementsByPoint(src_df0, group_by_nomvar=True, nomvar_out="TEST").compute()
        # [ReaderStd --input {sources[0]}] >> [AddElementsByPoint --outputFieldName TROPLONG]
...
def test_10(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    src_df1 = fstpy.StandardFileReader(source1).to_pandas()
    tt_uu_vv = pd.safe_concat([tt_uu_df, vv_df, src_df1])
    cols = ["nomvar", "grid", "ip1"]
    print(f"Input:  tt_uu_vv = \n{tt_uu_vv[cols]} \n")
    # compute AddElementsByPoint
    df = spookipy.AddElementsByPoint(tt_uu_vv, nomvar_out="ACCU").compute()
...
def test_11(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute AddElementsByPoint
    df = spookipy.AddElementsByPoint(tt_es_df, group_by_forecast_hour=True).compute()
...
def test_12(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = pd.safe_concat([src_df0, src_df1])

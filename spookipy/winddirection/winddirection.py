...
def wind_direction(uu: np.ndarray, vv: np.ndarray) -> np.ndarray:
    result = np.where((uu == 0.), np.where(vv >= 0., 180., 0.),
        ...
        try:
            ...
            for dependencies_df, _ in dependencies_list:
                ...
                if grtyp == 'Y':
                    ...
                    latGrid1  = rmn.gdll(subgridId1)['lat']
                    lonGrid1  = rmn.gdll(subgridId1)['lon']

                    latGrid2  = rmn.gdll(subgridId2)['lat']
                    lonGrid2  = rmn.gdll(subgridId2)['lon']

                    wd_df     = calc_wd_gridU(subgridId1, subgridId2, 
                                              uu_df, vv_df, wd_df, uv_df, 
                                              latGrid1, lonGrid1, latGrid2, lonGrid2)
                    ...
                    lat                = rmn.gdll(gdid)['lat']
                    lon                = rmn.gdll(gdid)['lon']
...
def calc_wd(gdid, uu_df, vv_df, wd_df, uv_df, lat, lon):
    ...
    for i in uu_df.index:
        ...
        rmn.c_gdwdfuv(gdid, uv, wd, uu, vv, lat, lon, lat.size)
        wd_df.at[i,'d'] = wd

...
class WindChill(Plugin):
    ...
    def __init__(self, df: pd.DataFrame, dependency_check=False, copy_input=False, reduce_df=True):
        self.plugin_mandatory_dependencies = [
            {
                "UV": {"nomvar": "UV", "unit": "km/h", "surface": True},
                "TT": {"nomvar": "TT", "unit": "celsius", "surface": True},
            }
        ]
        self.plugin_result_specifications = {
            "RE": {"nomvar": "RE", "label": "WNDCHL", "unit": "celsius", "level": 0, "ip1_kind": 2}
        }
    ...
    def prepare_groups(self):
        self.no_meta_df = fstpy.add_columns(self.no_meta_df, columns=["unit", "ip_info", "forecast_hour"])
    ...
    def compute(self) -> pd.DataFrame:
        ...
        try:
            dependencies_list = get_dependencies(
                self.groups, self.meta_df, "WindChill", self.plugin_mandatory_dependencies
            )

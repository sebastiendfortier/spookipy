...
class ArithmeticMeanByPoint(Plugin):
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            "--outputFieldName", type=str, dest="nomvar_out", help="Option to change the name of output field 'MEAN'."
        )
        ...
        parser.add_argument(
            ...
            type=str,
            dest="group_by",
            help=f"Options to group fields by attribute when performing calculation. Can have more than one separated by ',' no space allowed, possible values = {valid_group_by}. Ex --groupBy FORECAST_HOUR or --groupBy FORECAST_HOUR,FIELD_NAME,ENSEMBLE_MEMBER",
        )
        ...
        if parsed_arg["group_by"]:
            ...
            for gb in group_by_list:
                if gb not in valid_group_by:
                    raise ArithmeticMeanByPointError(
                        f"{gb} is not a valid option for --group_by, the valid choices are: {valid_group_by}"
                    )

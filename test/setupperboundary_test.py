...
def test_1(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = spookipy.SetUpperBoundary(src_df0, value=5.0).compute()
...
def test_2(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute SetUpperBoundary
    df = spookipy.SetUpperBoundary(src_df, value=0.0).compute()
...
def test_3(plugin_test_path):
    ...
    with pytest.raises(spookipy.SetUpperBoundaryError):
        _ = spookipy.SetUpperBoundary(src_df0, value=0.0, nomvar_out="TEST").compute()
...
def test_4(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute SetUpperBoundary
    df = spookipy.SetUpperBoundary(src_df, value=0.0, nomvar_out="TEST").compute()
    # [ReaderStd --input {sources[0]}] >> [Select --fieldName UU] >>

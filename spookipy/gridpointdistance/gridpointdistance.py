...
def calc_dist(lat1, lon1, lat2, lon2):
    ...
    return EARTH_RADIUS * math.acos(
        math.cos(radlat1) * math.cos(radlat2) * math.cos(radlon1 - radlon2) + math.sin(radlat1) * math.sin(radlat2)
    )
...
VEZCALCDIST_F: Final = np.vectorize(RmnInterface.calculate_distance_between_points)
...
def centered_distance(
    lats: np.ndarray, lons: np.ndarray, is_global: bool = False, grid_wraps: bool = False
) -> np.ndarray:
...
class GridPointDistance(Plugin):
    ...
    def validate_params(self):
        ...
        if self.difference_type not in ["centered", "forward", "backward"]:
            raise GridPointDistanceError(
                f"Invalid difference type specification! {self.difference_type} not in {['centered', 'forward', 'backward']}"
            )
    ...
    def compute(self):
            ...
            for _, grid_df in grid_groups:
                ...
                for grtyp, grtyp_df in grtyp_groups:
                    ...
                    if grtyp != "U":
                        ...
                        if self.difference_type == "centered":
                            if "x" in self.axis:
                                res1 = centered_distance(
                                    lat[:, 0 : int(lat.shape[1] / 2)],
                                    lon[:, 0 : int(lon.shape[1] / 2)],
                                    is_global,
                                    grid_wraps,
                                )
                                res2 = centered_distance(
                                    lat[:, int(lat.shape[1] / 2) :],
                                    lon[:, int(lon.shape[1] / 2) :],
                                    is_global,
                                    grid_wraps,
                                )
                            ...
                            if "y" in self.axis:
                                res1 = centered_distance(
                                    lat[:, 0 : int(lat.shape[1] / 2)].T, lon[:, 0 : int(lon.shape[1] / 2)].T
                                ).T
                                res2 = centered_distance(
                                    lat[:, int(lat.shape[1] / 2) :].T, lon[:, int(lon.shape[1] / 2) :].T
                                ).T
                            ...
                            if "x" in self.axis:
                                res1 = forward_distance(
                                    lat[:, 0 : int(lat.shape[1] / 2)],
                                    lon[:, 0 : int(lon.shape[1] / 2)],
                                    is_global,
                                    grid_wraps,
                                )
                                res2 = forward_distance(
                                    lat[:, int(lat.shape[1] / 2) :],
                                    lon[:, int(lon.shape[1] / 2) :],
                                    is_global,
                                    grid_wraps,
                            ...
                            if "x" in self.axis:
                                res1 = backward_distance(
                                    lat[:, 0 : int(lat.shape[1] / 2)],
                                    lon[:, 0 : int(lon.shape[1] / 2)],
                                    is_global,
                                    grid_wraps,
                                )
                                res2 = backward_distance(
                                    lat[:, int(lat.shape[1] / 2) :],
                                    lon[:, int(lon.shape[1] / 2) :],
                                    is_global,
                                    grid_wraps,
        ...
        return self.final_results(df_list, GridPointDistanceError, copy_input=self.copy_input, reduce_df=self.reduce_df)
    ...
    def parse_config(args: str) -> dict:
        ...
        :rtype: dict
        """
        parser = PluginParser(prog=GridPointDistance.__name__, parents=[Plugin.base_parser], add_help=False)
        parser.add_argument(
            "--differenceType",
            type=str,
            default="CENTERED",
            choices=["CENTERED", "FORWARD", "BACKWARD"],
            dest="difference_type",
            help="Type of difference.",
        )
        parser.add_argument(
            "--axis",
            type=str,
            required=True,
            help="Comma separated list of axis on which the differences will be calculated.",
        )

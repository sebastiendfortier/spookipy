...
def validate_list_of_str(param):
    ...
    if not isinstance(param, list):
        raise TimeIntervalDifferenceError(f'{str(param)} needs to be a list of str')
    for n in param:
...
def validate_list_of_times(param):
    if isinstance(param, datetime.timedelta):
        param = [param]
    if not isinstance(param, list):
        raise TimeIntervalDifferenceError(f'{str(param)} needs to be a list of datetime.timedelta')
    for n in param:
        if not isinstance(n, datetime.timedelta):
            raise TimeIntervalDifferenceError(f'{str(param)} needs to be a list of datetime.timedelta')
        if n == datetime.timedelta():
            raise TimeIntervalDifferenceError('value is not valid') 
    return param
...
def validate_list_of_tuples_of_times(param):
    if isinstance(param, tuple) and len(param) == 2:
        param = [param]
    if not isinstance(param, list):
        raise TimeIntervalDifferenceError(f'{str(param)} is not a list, {str(param)} needs to be a list of tuple of 2 datetime.timedelta')
    for n in param:
        ...
        if n[0] >= n[1]:
            raise TimeIntervalDifferenceError(f'{str(param)} value is not valid')  
    return param        

...
class ArithmeticStandardDeviationByPoint(Plugin):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        r = self.final_results(
            df_list, ArithmeticStandardDeviationByPointError, copy_input=self.copy_input, reduce_df=self.reduce_df
        )
    ...
    def parse_config(args: str) -> dict:
        ...
        """
        parser = PluginParser(
            prog=ArithmeticStandardDeviationByPoint.__name__, parents=[Plugin.base_parser], add_help=False
        )
        ...
        parser.add_argument(
            ...
            choices=[0, 1],
            type=int,
            help="Par défaut 1 (pour écart-type échantillion). Pour calculer l'écart-type de la population, il faut ajuster cette variable à 0. Une seule valeur entière 0 ou 1.",
        )
        parser.add_argument(
            "--ignore_mask",
            action="store_true",

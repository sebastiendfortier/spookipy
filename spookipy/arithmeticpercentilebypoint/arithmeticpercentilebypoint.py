...
class ArithmeticPercentileByPoint(Plugin):
    ...
    def prepare_groups(self):
        for p in self.percentiles:
            if p < 0.0 or p > 100.0:
                raise ArithmeticPercentileByPointError(f'Percentiles need to be between 0 and 100, invalid percentile {p}.')
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, df_group in self.groups:
            ...
            for l,level_df in level_groups:
                ...
                all_ensemble_data_result = np.percentile(all_ensemble_data, self.percentiles, axis=0, method=self.method)
                ...
                for x in all_ensemble_data_result:
                    ...
                    percentile_res_df = create_empty_result(
                        level_df,
                        {"label": make_label(self.percentiles[i]), "ensemble_member":"ALL",
        ...
        r = self.final_results(df_list, ArithmeticPercentileByPointError,
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument('--percentiles', required=True, type=str,help='Liste des percentiles (float) à calculer. De 0 à 100 inclusivement')
        ...
        parser.add_argument('--method', default='linear',choices=['inverted_cdf', 'averaged_inverted_cdf', 'closest_observation', 'interpolated_inverted_cdf', 'hazen', 'weibull', 'linear', 'median_unbiased', 'normal_unbiased',], help="Methode d'interpolation de numpy percentile")

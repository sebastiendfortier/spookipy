...
def base_dict():
    base = {'datev':0,'path':None,'typvar':'X', 'ni':1,'nj':1,'nk':1,'ip1':0,'ip2':0,'ip3':0, 'deet':0,'npas':0,'datyp':5,'nbits':32, 'grtyp':'L','ig1':100,'ig2':100,'ig3':9000,'ig4':0,'date_of_origin':date_of_origin()} 
    lat = base.copy()
    lat['nomvar'] = 'LAT'
    lon = base.copy()
    lon['nomvar'] = 'LON'
...
def simple_input_df():
    ...
    lat['ni'] = len(lat['d']) # this should be nj
...
def test_regtest_3(plugin_test_dir,latlon_df):
    ...
    not_vs_df = df.query('nomvar!="VS"')
    vs_df = df.query('nomvar=="VS"')

    vs_df['etiket'] = 'XVSHEAR_X'
    
    df = pd.concat([vs_df,not_vs_df],ignore_index=True)
...
#     src_df0 = src_df0.query('nomvar in ["GZ","UU","VV","TT",">>","^^","!!"]')
...
def test_regtest_13(plugin_test_dir,latlon_df):
    ...
    #[ReaderStd --input {sources[0]}] >> [ReaderStd --input {sources[1]}] >> [ReaderCsv --input {sources[2]}] >> [spooki.InterpolationHorizontalPoint] >> [Zap --metadataZappable --dateOfOrigin 20110210T215210 --doNotFlagAsZapped] >> [WriterStd --output {destination_path} --makeIP1EncodingWorkWithTests]
...
def test_regtest_14(plugin_test_dir):
    ...
    src_df0 = src_df0.query('nomvar in ["2Z",">>","^^","!!"]')

    source1 = plugin_test_dir + "latlong_stn_ALL.fst"
    src_df1 = StandardFileReader(source1,load_data=True).to_pandas()
    src_df1['ip1']=0
...
def test_regtest_15(plugin_test_dir):
    ...
    src_df0 = src_df0.query('nomvar in ["SN",">>","^^","!!"]')
...
def test_regtest_16(plugin_test_dir,simple_input_df):
    ...
    src_df0 = src_df0.query('nomvar in ["TT","^>","!!"]')

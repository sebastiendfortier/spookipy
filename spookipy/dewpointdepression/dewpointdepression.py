...
class DewPointDepression(Plugin):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, current_group in self.groups:
            # print('current_group\n',current_group[['nomvar','typvar','etiket','dateo','forecast_hour','ip1_kind','grid']].to_string())
            if self.rpn:
                sys.stdout.write('DewPointDepression - Checking rpn dependencies\n')
            ...
            if dependencies_df.empty:
                sys.stdout.write('DewPointDepression - No matching dependencies found for this group \n%s\n'%current_group[['nomvar','typvar','etiket','dateo','forecast_hour','ip1_kind','grid']])
                continue
            else:
                sys.stdout.write('DewPointDepression - Matching dependencies found for this group \n%s\n'%current_group[['nomvar','typvar','etiket','dateo','forecast_hour','ip1_kind','grid']])

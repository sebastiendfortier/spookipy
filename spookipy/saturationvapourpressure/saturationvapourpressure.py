...
class SaturationVapourPressure(Plugin):
    ...
    def __init__(
        ...
        self.plugin_mandatory_dependencies_rpn = [{"TT": {"nomvar": "TT", "unit": "kelvin"}}]

        self.plugin_mandatory_dependencies = [{"TT": {"nomvar": "TT", "unit": "celsius"}}]
        self.plugin_result_specifications = {
            "SVP": {"nomvar": "SVP", "label": "SVPRES", "unit": "hPa", "nbits": 16, "datyp": 1},
        }
    ...
    def prepare_groups(self):
        ...
        mandatory_temp_phase_switch_when_using_ice_water_phase_both(
            SaturationVapourPressureError, self.explicit_params, self.ice_water_phase, self.rpn, True
        )
            ...
            for dependencies_df, _ in dependencies_list:
                ...
                if self.rpn:
                    ...
                    for i in tt_df.index:
                        tt = tt_df.at[i, "d"]
                        svp_df.at[i, "d"] = svp_from_tt(
                            tt - TDPACK_OFFSET_FIX,
                            tpl=(self.temp_phase_switch if self.ice_water_phase != "water" else -40),
                            swph=self.ice_water_phase == "both",
                        ).astype(np.float32)

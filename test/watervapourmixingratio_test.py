...
def test_regtest_3(plugin_test_dir):
    ...
    dpd_df = DewPointDepression(src_df0,ice_water_phase='water',rpn=True).compute()
    tt_df = fstpy.select_with_meta(src_df0,['TT'])
    df = pd.concat([tt_df,dpd_df],ignore_index=True)
    #compute WaterVapourMixingRatio
...
def test_regtest_4(plugin_test_dir):
    ...
    tt_df = fstpy.select_with_meta(src_df0,['TT'])
    tdp_df = TemperatureDewPoint(src_df0,ice_water_phase='water',rpn=True).compute()
    df = pd.concat([tt_df,tdp_df],ignore_index=True)
    #compute WaterVapourMixingRatio
    df = spooki.WaterVapourMixingRatio(df,ice_water_phase='both',temp_phase_switch=-40,rpn=True).compute()
    #[ReaderStd --input {sources[0]}] >> 
...
def test_regtest_5(plugin_test_dir):
    ...
    #compute WaterVapourMixingRatio
    df = spooki.WaterVapourMixingRatio(src_df0).compute()
    #[ReaderStd --input {sources[0]}] >> 
    # [Select --fieldName HU] >>
...
def test_regtest_9(plugin_test_dir):
    ...
    with pytest.raises(spooki.WaterVapourMixingRatioError):
        _ = spooki.WaterVapourMixingRatio(src_df0).compute()

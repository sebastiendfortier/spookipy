...
def test_2(plugin_test_path):
    ...
    # compute HumiditySpecific
    with pytest.raises(spookipy.HumiditySpecificError):
        _ = spookipy.HumiditySpecific(src_df0, temp_phase_switch=-30, temp_phase_switch_unit="celsius").compute()
...
def test_3(plugin_test_path):
    ...
    with pytest.raises(spookipy.HumiditySpecificError):
        _ = spookipy.HumiditySpecific(
            src_df0, ice_water_phase="water", temp_phase_switch=-30, temp_phase_switch_unit="celsius"
        ).compute()
...
def test_6(plugin_test_path):
    ...
    with pytest.raises(spookipy.HumiditySpecificError):
        _ = spookipy.HumiditySpecific(
            src_df0, ice_water_phase="both", temp_phase_switch=-10, temp_phase_switch_unit="kelvin"
        ).compute()
...
def test_9(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute HumiditySpecific
    df = spookipy.HumiditySpecific(tthr_df, ice_water_phase="water", rpn=True).compute()
    # [ReaderStd --input {sources[0]}] >>
    # [Select --fieldName TT,HR] >>
...
def test_10(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute HumiditySpecific
    es_df = spookipy.DewPointDepression(tthu_df, ice_water_phase="water").compute()
    tt_df = fstpy.select_with_meta(tthu_df, ["TT"])
    dpdtt_df = pd.safe_concat([es_df, tt_df])
...
def test_11(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    warnings.filterwarnings("ignore", category=UserWarning, module="spookipy")
    # compute HumiditySpecific
    es_df = spookipy.DewPointDepression(tthu_df, ice_water_phase="water", rpn=True).compute()

    tt_df = fstpy.select_with_meta(tthu_df, ["TT"])

    ttes_df = pd.safe_concat([es_df, tt_df])

    df = spookipy.HumiditySpecific(ttes_df, ice_water_phase="water", rpn=True).compute()
    # [ReaderStd --input {sources[0]}] >>
    #  [Select --fieldName TT,HU] >>
...
def test_12(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    es_df = spookipy.DewPointDepression(tthu_df, ice_water_phase="water").compute()
    estt_df = pd.safe_concat([es_df, tt_df])

    td_df = spookipy.TemperatureDewPoint(estt_df, ice_water_phase="water").compute()
    tttd_df = pd.safe_concat([td_df, tt_df])
...
def test_13(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute HumiditySpecific
    df = spookipy.HumiditySpecific(tttd_df, ice_water_phase="water", rpn=True).compute()
    # [ReaderStd --input {sources[0]}] >>
    # [Select --fieldName TT,HU] >>
    # ([DewPointDepression --iceWaterPhase WATER --RPN] + [Select --fieldName TT]) >>
    # ([Select --fieldName TT] + [TemperatureDewPoint --iceWaterPhase WATER]) >>
...
def test_14(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    es_df = spookipy.DewPointDepression(ttes_df, ice_water_phase="water").compute()
    ttes_df = pd.safe_concat([es_df, tt_df])
...
def test_15(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    td_df = spookipy.TemperatureDewPoint(
        estt_df, ice_water_phase="both", temp_phase_switch=-40, temp_phase_switch_unit="celsius"
    ).compute()
    tttd_df = pd.safe_concat([td_df, tt_df])
...
def test_16(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = spookipy.HumiditySpecific(tttd_df, copy_input=True).compute()
...
def test_17(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    new_df = pd.safe_concat([meta_df, tthr_df])

    # compute HumiditySpecific
    df = spookipy.HumiditySpecific(
        new_df, ice_water_phase="both", temp_phase_switch=273, temp_phase_switch_unit="celsius"
    ).compute()
...
def test_18(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    warnings.filterwarnings("ignore", category=UserWarning, module="spookipy")
    es_df = spookipy.DewPointDepression(ttes_df, ice_water_phase="water", rpn=True).compute()
    ttes_df = pd.safe_concat([es_df, tt_df])

    qv_df = spookipy.WaterVapourMixingRatio(ttes_df, rpn=True).compute()

    # compute HumiditySpecific
    df = spookipy.HumiditySpecific(qv_df, ice_water_phase="water", rpn=True).compute()
...
def test_19(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    warnings.filterwarnings("ignore", category=UserWarning, module="spookipy")
    # compute
    df = spookipy.HumiditySpecific(tthr_df, ice_water_phase="both", rpn=True).compute()

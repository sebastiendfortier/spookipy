            ...
            if self.rpn:
                ...
                if self.option==1:
                    ...
                    for i in vppr_df.index:
                        hu = hu_df.at[i,'d']
                        px = px_df.at[i,'d']
                        vppr_df.at[i,'d'] = rpn_calc_vapour_pressure_hu(hu, px).astype(float32)
                    ...
                    for i in vppr_df.index:
                        qv = qv_df.at[i,'d']
                        px = px_df.at[i,'d']
                        vppr_df.at[i,'d'] = calc_vapour_pressure_qv(qv, px).astype(float32)
                    ...
                    hu_df = HumiditySpecific(pd.concat([current_fhour_group,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch,temp_phase_switch_unit=self.temp_phase_switch_unit,rpn=True).compute()
                    ...
                    for i in vppr_df.index:
                        ...
                        es = es_df.at[i,'d']
                        td = calc_temperature_dew_point_es(tt, es)
                        vppr_df.at[i,'d'] = rpn_calc_vapour_pressure_td(td, tt, self.temp_phase_switch, self.ice_water_phase=='both').astype(float32)
                else:
                    print('option 5')
                ...
                if self.option==1:
                    ...
                    for i in vppr_df.index:
                        hu = hu_df.at[i,'d']
                        px = px_df.at[i,'d']
                        vppr_df.at[i,'d'] = calc_vapour_pressure_hu(hu, px).astype(float32)
                    ...
                    level_intersection_df = fstpy.load_data(level_intersection_df)
                    svp_df = level_intersection_df.query( 'nomvar=="SVP"').reset_index(drop=True)
                    hr_df = level_intersection_df.query( 'nomvar=="HR"').reset_index(drop=True)
                    vppr_df = create_empty_result(svp_df,self.plugin_result_specifications['VPPR'],copy=True)
                    for i in vppr_df.index:
                        hr = hr_df.at[i,'d']
                        svp = svp_df.at[i,'d']
                        vppr_df.at[i,'d'] = calc_vapour_pressure_hr(hr,svp).astype(float32)
                    ...
                    for i in vppr_df.index:
                        ...
                        es = es_df.at[i,'d']
                        td = calc_temperature_dew_point_es(tt, es)
                        vppr_df.at[i,'d'] = calc_vapour_pressure_td(td, tt, self.temp_phase_switch, self.ice_water_phase=='both').astype(float32)
                else:
                    print('option 5')

...
from ..utils import (
    create_empty_result,
    get_list_of_forecast_hours,
    initializer,
    to_numpy,
    validate_list_of_nomvar,
    validate_list_of_times,
    validate_list_of_tuples_of_times,
    validate_nomvar,
)
...
class TimeIntervalMinMax(Plugin):
    ...
    def prepare_groups(self):
        ...
        if (self.nomvar is None) or (self.forecast_hour_range is None):
            raise TimeIntervalMinMaxError("One of the mandatory parameter (nomvar, forecast_hour_range) is None")
        ...
        self.df_without_intervals = self.no_meta_df.loc[
            (self.no_meta_df.interval.isna()) & (self.no_meta_df.nomvar.isin(self.nomvar))
        ].reset_index(drop=True)

        self.df_with_intervals = self.no_meta_df.loc[
            (~self.no_meta_df.interval.isna()) & (self.no_meta_df.nomvar.isin(self.nomvar))
        ].reset_index(drop=True)

        self.groups_without_interval = self.df_without_intervals.groupby(["grid", "nomvar", "level", "vctype"])
        self.groups_with_interval = self.df_with_intervals.groupby(["grid", "nomvar", "level", "vctype"])
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, current_group in self.groups_with_interval:
            ...
            for forecast_hours in self.forecast_hours:
                ...
                interval_df = current_group.loc[
                    current_group.lower_bound.astype("int32").between(b_inf, b_sup, inclusive="both")
                ]

                if interval_df.empty:
                    interval_df = current_group.loc[
                        current_group.upper_bound.astype("int32").between(b_inf, b_sup, inclusive="both")
                    ]
                    if interval_df.empty:
        ...
        for _, current_group in self.groups_without_interval:
            ...
            for forecast_hours in self.forecast_hours:
                ...
                interval_df = current_group.loc[
                    current_group.forecast_hour.dt.total_seconds()
                    .astype("int32")
                    .between(b_inf, b_sup, inclusive="both")
                ]
        ...
        return self.final_results(df_list, TimeIntervalMinMaxError, copy_input=False)
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            "--interval", type=str, help="List of each time range used for the minimum/maximum calculation"
        )
        parser.add_argument(
            "--rangeForecastHour",
            required=True,
            type=str,
            dest="forecast_hour_range",
            help="List of time ranges in hours.",
        )
        parser.add_argument(
            "--step",
            type=str,
            help="List of the time steps in hours between successive start times within each time range.",
        )
        parser.add_argument("--outputFieldNameMax", type=str, dest="nomvar_max", help="List of names of maximum field.")
        parser.add_argument(
            "--outputFieldNameMin", type=str, dest="nomvar_min", help="List of names if minimum fields."
        )
        parser.add_argument(
            "--type",
            type=str,
            required=True,
            choices=["MIN", "MAX", "BOTH"],
            help="Calculation of minimum and/or maximum.",
        )
        ...
        parsed_arg["forecast_hour_range"] = apply_lambda_to_list(
            parsed_arg["forecast_hour_range"].split(","), lambda a: convert_time_range(a)
        )

        parsed_arg["nomvar"] = parsed_arg["nomvar"].split(",")
        apply_lambda_to_list(
            parsed_arg["nomvar"], lambda a: validate_nomvar(a, "TimeIntervalMinMax", TimeIntervalMinMaxError)
        )

        if parsed_arg["nomvar_max"] is not None:
            parsed_arg["nomvar_max"] = parsed_arg["nomvar_max"].split(",")
            apply_lambda_to_list(
                parsed_arg["nomvar_max"],
                lambda a: True if a is None else validate_nomvar(a, "TimeIntervalMinMax", TimeIntervalMinMaxError),
            )
        if parsed_arg["nomvar_min"] is not None:
            parsed_arg["nomvar_min"] = parsed_arg["nomvar_min"].split(",")
            apply_lambda_to_list(
                parsed_arg["nomvar_min"],
                lambda a: True if a is None else validate_nomvar(a, "TimeIntervalMinMax", TimeIntervalMinMaxError),
            )
...
def create_result_container(df, b_inf, b_sup, nomvar):
    ...
    res_df = create_empty_result(df, {"nomvar": nomvar, "label": "TIMNMX", "interval": inter, "npas": npas})
    return res_df


def check_for_negative_values(arr, location):
    if np.any(np.where(arr < 0.0, True, False)):
        logging.warning(
            f"Found a negative value in the {location}! Probable cause is loss of precision when converting to float computational type"
        )

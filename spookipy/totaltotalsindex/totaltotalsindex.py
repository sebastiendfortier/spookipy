...
class TotalTotalsIndex(Plugin):
    """Calculates the Total Totals index, a severe weather index used for forecasting thunderstorms.
    ...
    def __init__(self, df: pd.DataFrame, copy_input=False, reduce_df=True):
        ...
        self.plugin_result_specifications = {
            "TTI": {"nomvar": "TTI", "label": "TOTALI", "unit": "celsius", "ip1_kind": 2, "level": 0}
        }
    ...
    def compute(self) -> pd.DataFrame:
        ...
        dependencies_list = get_dependencies(
            self.groups, self.meta_df, "TotalTotalsIndex", self.plugin_mandatory_dependencies
        )
        ...
        return self.final_results(df_list, TotalTotalsIndexError, copy_input=self.copy_input, reduce_df=self.reduce_df)

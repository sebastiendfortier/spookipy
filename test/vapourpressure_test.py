...
def test_regtest_1(plugin_test_dir):
    ...
    with pytest.raises(spooki.VapourPressureError):
        _ = spooki.VapourPressure(src_df0, ice_water_phase='both', temp_phase_switch=-30, temp_phase_swotch_unit='G').compute()
...
def test_regtest_2(plugin_test_dir):
    ...
    #compute VapourPressure
    with pytest.raises(spooki.VapourPressureError):
        _ = spooki.VapourPressure(src_df0, ice_water_phase='both', temp_phase_switch=-273.16, temp_phase_swotch_unit='kelvin').compute()
...
def test_regtest_3(plugin_test_dir):
    ...
    #compute VapourPressure
    with pytest.raises(spooki.VapourPressureError):
        _ = spooki.VapourPressure(src_df0, ice_water_phase='both', temp_phase_switch=273.17, temp_phase_swotch_unit='kelvin').compute()
...
def test_regtest_4(plugin_test_dir):
    ...
    #compute VapourPressure
    with pytest.raises(spooki.VapourPressureError):
        _ = spooki.VapourPressure(src_df0, ice_water_phase='invalide', temp_phase_switch=273.17, temp_phase_swotch_unit='kelvin').compute()
...
def test_regtest_5(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(tthu_df).compute()
...
def test_regtest_6(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(tthu_df, rpn=True).compute()
    #[ReaderStd --input {sources[0]}] >> 
    # [Select --fieldName TT,HU] >>
    #  [VapourPressure --RPN] >>
    #  [WriterStd --output {destination_path} --noMetadata --ignoreExtended]
...
def test_regtest_7(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(tthr_df).compute()
    #[ReaderStd --input {sources[0]}] >> 
...
def test_regtest_9(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(ttes_df).compute()
    #[ReaderStd --input {sources[0]}] >> 
...
def test_regtest_11(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(src_df0).compute()
    #[ReaderStd --input {sources[0]}] >> 
    # [VapourPressure ] >> 
...
def test_regtest_12(plugin_test_dir):
    ...
    #compute VapourPressure
    df = spooki.VapourPressure(src_df0,rpn=True).compute()
    #[ReaderStd --input {sources[0]}] >> 

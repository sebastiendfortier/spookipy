...
class SetConstantValue(Plugin):
    ...
    def __init__(
        ...
        if self.nomvar_out is not None:
            validate_nomvar(self.nomvar_out, "SetConstantValue", SetConstantValueError)

        self.plugin_result_specifications = {"ALL": {"label": "SETVAL", "unit": "1"}}
    ...
    def prepare_groups(self):
        ...
        if self.nomvar_out is not None:
            self.groups_tmp = self.no_meta_df.groupby(by=["grid", "datev", "dateo", "level"])
            for _, current_group in self.groups_tmp:
                if len(current_group.index) > 1:
                    raise SetConstantValueError(' More than one input field, cannot use "nomvar_out" ')
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, df in self.groups:
            ...
            res_df = create_empty_result(df, self.plugin_result_specifications["ALL"], all_rows=True)
        ...
        return self.final_results(df_list, SetConstantValueError, copy_input=False, reduce_df=self.reduce_df)
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            "--bidimensional",
            action="store_true",
            dest="bi_dimensionnal",
            help="2D field output even if the input is a 3D field",
        )

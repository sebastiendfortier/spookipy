...
class HumidityRelativeWeightedMean(Plugin):
    ...
    @initializer
    def __init__(
        self, df: pd.DataFrame, capped_value: float = None, dependency_check=False, copy_input=False, reduce_df=True
    ):
        self.plugin_mandatory_dependencies = [
            {
                ...
                "HU1": {"nomvar": "HU", "unit": "kg/kg", "level": 1000, "ip1_pkind": "mb", "select_only": True},
                "HU2": {"nomvar": "HU", "unit": "kg/kg", "level": 925, "ip1_pkind": "mb", "select_only": True},
                "HU3": {"nomvar": "HU", "unit": "kg/kg", "level": 850, "ip1_pkind": "mb", "select_only": True},
                "HU4": {"nomvar": "HU", "unit": "kg/kg", "level": 700, "ip1_pkind": "mb", "select_only": True},
                "HU5": {"nomvar": "HU", "unit": "kg/kg", "level": 500, "ip1_pkind": "mb", "select_only": True},
            }
        ]

        self.plugin_result_specifications = {"HR": {"label": "HRWAVG", "unit": "%"}}
    ...
    def validate_params_and_input(self):
        if self.capped_value is not None:
            if self.capped_value <= 0:
                raise HumidityRelativeWeightedMeanError(f"The capped_value is negative =  {self.capped_value} !")
    ...
    def compute(self) -> pd.DataFrame:
        ...
        try:
            ...
            for dependencies_df, option in dependencies_list:
                ...
                tt_reduit_df = pd.safe_concat([tt1000_df, tt925_df, tt850_df, tt700_df, tt500_df])
                ...
                if not (self.capped_value is None):
                    hrl_df.at[0, "d"] = np.where(
                        hrl_df.at[0, "d"] > self.capped_value, self.capped_value, hrl_df.at[0, "d"]
                    )
                    hrl_df = reshape_arrays(hrl_df)
                    hrl_df = dataframe_arrays_to_dask(hrl_df)

                    hrm_df.at[0, "d"] = np.where(
                        hrm_df.at[0, "d"] > self.capped_value, self.capped_value, hrm_df.at[0, "d"]
                    )
            ...
            return self.final_results(
                df_list,
                HumidityRelativeWeightedMeanError,
    ...
    def compute_hus(self, tt_df, td_df):
        ...
        hus_df = HumiditySpecific(
            pd.safe_concat([tt_df, td_df]), dependency_check=self.dependency_check, reduce_df=False
        ).compute()

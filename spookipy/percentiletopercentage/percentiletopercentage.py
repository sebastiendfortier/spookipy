...
def calculate_percentage(arr: np.ndarray, threshold: float, percentile_step: list) -> float:
    ...
    if equal_to[0].size > 0:
        ...
        interpolated_percentile_step = (
            diff_percentile_step / diff_arr * (threshold - arr[smaller_than[0][-1]])
            + percentile_step[smaller_than[0][-1]]
        )
...
class PercentileToPercentage(Plugin):
    ...
    :param threshold: Threshold value , required
    :type threshold: float, optional
    :param operator: Operator, possible option are ge and le , required
    :type operator: str, optional
    :param label: Output label name, defaults to combination of operator and treshold
    ...
    @initializer
    def __init__(self, df: pd.DataFrame, threshold: float, operator: str, label: str = None, reduce_df=True):
    ...
    def prepare_groups(self):
        ...
        field_df = self.no_meta_df.loc[
            (~self.no_meta_df.typvar.isin(["@@", "!@"])) & (self.no_meta_df.label.str.contains(r"\d"))
        ]
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for forecast_hour, group_df in self.groups:
            ...
            if self.operator == "ge":
                percentile_field = np.apply_along_axis(
                    field_to_percentage_ge, 0, group_field_stacked, self.threshold, percentiles
                )
            elif self.operator == "le":
                percentile_field = np.apply_along_axis(
                    field_to_percentage_le, 0, group_field_stacked, self.threshold, percentiles
                )
            else:
                raise PercentileToPercentageError(
                    f'WHILE CALLING PercentileToPercentage - USE OF INVALID OPERATOR: "{self.operator}" -  VALID PARAMETER VALUES FOR --operator ARE: [ge|le]'
                )
        ...
        return self.final_results(df_list, PercentileToPercentageError, copy_input=False, reduce_df=self.reduce_df)
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument("--label", type=str, default="", help="Label of the output field.")
        parser.add_argument("--threshold", required=True, type=float, help="Threshold value.")
        parser.add_argument("--operator", required=True, type=str, choices=["ge", "le"], help="Comparison operator.")
...
def make_label(threshold, operator) -> str:
    threshold = str(threshold)
    if "." in threshold:
        threshold = threshold.rstrip("0")
        # Check if the string ends with a decimal point and remove it if necessary
        if threshold.endswith("."):
            threshold = threshold[:-1]

    label = operator + threshold + "_____"
    label = label.upper()

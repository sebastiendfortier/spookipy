        ...
        for _, current_fhour_group in self.fhour_groups:
            if self.rpn:
                if self.option==1:
                    ...
                    hu_df = current_fhour_group.query( 'nomvar=="HU"').reset_index(drop=True)
                    ...
                    for i in hr_df.index:
                        ...
                        px = px_df.at[i,'d']
                        hu = hu_df.at[i,'d']
                        hr_df.at[i,'d'] = rpn_calc_humidity_relative_hu(hu, tt, px, self.ice_water_phase=='both')
                    ...
                    hr_df = create_empty_result(tt_df,self.plugin_result_specifications['HR'],copy=True)
                    hu_df = HumiditySpecific(pd.concat([qv_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,rpn=True).compute()
                    ...
                    hr_df = create_empty_result(tt_df,self.plugin_result_specifications['HR'],copy=True)
                    es_df = DewPointDepression(pd.concat([tt_df,td_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,rpn=True).compute()
                    ...
                    for i in hr_df.index:
                        ...
                        px = px_df.at[i,'d']
                        es = es_df.at[i,'d']
                        hr_df.at[i,'d'] = rpn_calc_humidity_relative_es(es, tt, px, self.ice_water_phase=='both')
                        # hr_df.at[i,'d'] = calc_humidity_relative_svp_vppr(svp,vppr)
            else: # not rpn
                if self.option==1:
                    print('option 1')
                    ...
                    vppr_df = VapourPressure(pd.concat([px_df,hu_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    ...
                    svp_df = SaturationVapourPressure(pd.concat([tt_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    svp_df = svp_df.loc[svp_df.nomvar=='SVP'].reset_index(drop=True)
                    vppr_df = VapourPressure(pd.concat([px_df,qv_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    vppr_df = vppr_df.loc[vppr_df.nomvar=='VPPR'].reset_index(drop=True)
                    ...
                    svp_df = SaturationVapourPressure(pd.concat([tt_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    svp_df = svp_df.loc[svp_df.nomvar=='SVP'].reset_index(drop=True)
                    vppr_df = VapourPressure(pd.concat([tt_df,td_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    vppr_df = vppr_df.loc[vppr_df.nomvar=='VPPR'].reset_index(drop=True)
                    ...
                    svp_df = SaturationVapourPressure(pd.concat([tt_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    svp_df = svp_df.loc[svp_df.nomvar=='SVP'].reset_index(drop=True)
                    vppr_df = VapourPressure(pd.concat([es_df,self.meta_df],ignore_index=True),ice_water_phase=self.ice_water_phase,temp_phase_switch=self.temp_phase_switch).compute()
                    vppr_df = vppr_df.loc[vppr_df.nomvar=='VPPR'].reset_index(drop=True)
                    for i in hr_df.index:
                        svp = svp_df.at[i,'d']
                        vppr = vppr_df.at[i,'d']

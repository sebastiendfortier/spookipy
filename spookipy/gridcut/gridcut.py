...
class GridCut(Plugin):
    ...
    def validate_input(self):
        ...
        self.meta_df = self.df.loc[self.df.nomvar.isin(["^>", "!!", "!!SF", "HY"])].reset_index(drop=True)

        self.validate_grid()

        self.df = self.df.loc[~self.df.nomvar.isin(["^>", ">>", "^^", "!!", "!!SF", "HY"])].reset_index(drop=True)

    def validate_grid(self):
        tictac_df = self.meta_df.loc[self.meta_df.nomvar == "^>"].reset_index(drop=True)
        ...
        if len(self.grid_tag) != 3:
            ...
            for elem in self.grid_tag:
                if not isinstance(elem, int) or elem <= 0:
                    raise GridCutError(
                        f"grid_tag must have 3 positives integers. '{str(elem)}' is not a positive integer."
                    )
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for i in cp_df.index:
            ...
            cp_df.at[i, "d"] = cp_df.at[i, "d"][
                self.start_point[0] : self.end_point[0] + 1, self.start_point[1] : self.end_point[1] + 1
            ]
        ...
        res_df = pd.safe_concat([cp_df, self.meta_df, cptic_df, cptac_df])
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            "--startPoint", type=str, required=True, dest="start_point", help="Starting point of the selected matrix."
        )
        parser.add_argument(
            "--endPoint", type=str, required=True, dest="end_point", help="Ending point of the selected matrix."
        )
        parser.add_argument(
            "--gridTag",
            type=str,
            required=False,
            dest="grid_tag",
            help="The ip[1-3]/ig[1-3] values to identify the resulting grid. ex: --gridTag 110,240,3. Default will be to only increase ip3/ig3 by one",
        )
        ...
        if (
            parsed_arg["start_point"][0] < 0
            or parsed_arg["start_point"][1] < 0
            or parsed_arg["end_point"][0] < 0
            or parsed_arg["end_point"][1] < 0
        ):
        ...
        if parsed_arg["grid_tag"] != None:
            try:
                ...
                raise GridCutError(
                    "gridTag parameter must have 3 positives integers separated by comma. ex: --gridTag 4,1000,1002"
                )

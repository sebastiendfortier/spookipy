...
class SubtractElementsVertically(Plugin):
    ...
    @initializer
    def __init__(self, df: pd.DataFrame, direction: str = "ascending", nomvar_out: str = None, reduce_df=True):
    ...
    def validate_params_and_input(self):
        if self.direction not in ["ascending", "descending"]:
            raise SubtractElementsVerticallyError(
                "Invalid value '{self.direction}' for direction, valid values are {['ascending', 'descending']}"
            )
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, nomvar_df in groups:
            ...
            res_df = create_result_container(nomvar_df, last_level, first_level, self.plugin_result_specifications)
        ...
        return self.final_results(df_list, SubtractElementsVerticallyError, copy_input=False, reduce_df=self.reduce_df)

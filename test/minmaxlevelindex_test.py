...
def test_1(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # df = src_df0
    df = spookipy.MinMaxLevelIndex(src_df0, nomvar="TT", min=True, ascending=True, nomvar_min_idx="IND").compute()
...
def test_2(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(src_df0, nomvar="TT", max=True, ascending=True, nomvar_max_idx="IND").compute()
    # [ReaderStd --input {sources[0]}] >> [Select --fieldName TT] >>
...
def test_3(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = spookipy.MinMaxLevelIndex(
        src_df0, nomvar="TT", min=True, max=True, ascending=False, nomvar_min_idx="MIN", nomvar_max_idx="MAX"
    ).compute()
    # [ReaderStd --input {sources[0]}] >> [Select --fieldName TT] >>
...
def test_4(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(
        src_df0, nomvar="GZ", min=True, max=True, nomvar_min_idx="MIN", nomvar_max_idx="MAX"
    ).compute()
...
def test_5(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(
        src_df0, nomvar="GZ", min=True, max=True, ascending=False, nomvar_min_idx="MIN", nomvar_max_idx="MAX"
    ).compute()
    #     # [ReaderStd --input {sources[0]}] >> [Select --fieldName GZ] >>
    #     # [MinMaxLevelIndex --minMax BOTH --direction ASCENDING --outputFieldName1 MIN --outputFieldName2 MAX] >>
    #     # [WriterStd --output {destination_path} --encodeIP2andIP3]",
...
def test_6(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(
        src_df0, nomvar="UU", min=True, max=True, ascending=False, nomvar_min_idx="MIN", nomvar_max_idx="MAX"
    ).compute()
...
def test_9(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="ICGA", max=True, bounded=True, nomvar_max_idx="IND").compute()
...
def test_10(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="TT", bounded=True, min=True, max=True).compute()
...
def test_11(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="TT", bounded=True, ascending=False, min=True, max=True).compute()
    #     # [ReaderStd --input {sources[0]}] >> [Select --fieldName TT,KBAS,KTOP] >>
    #     # [MinMaxLevelIndex --bounded --minMax BOTH --direction ASCENDING] >>
    #     # [WriterStd --output {destination_path} --encodeIP2andIP3]",
...
def test_12(plugin_test_path):
    ...
    with pytest.raises(spookipy.MinMaxLevelIndexError):
        spookipy.MinMaxLevelIndex(src_df0, nomvar="TT", bounded=True).compute()
...
def test_13(plugin_test_path):
    ...
    with pytest.raises(spookipy.DependencyError):
        spookipy.MinMaxLevelIndex(src_df, nomvar="UU", bounded=True).compute()
...
def test_20(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    src_df = pd.safe_concat([src_df0, src_df1])
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="TT", value_to_return=True).compute()
...
def test_21(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="TT", bounded=True).compute()
...
def test_22(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    # compute spookipy.MinMaxLevelIndex
    df = spookipy.MinMaxLevelIndex(
        src_df0, nomvar="TT", min=True, ascending=True, nomvar_min_idx="IND", copy_input=True
    ).compute()
...
def test_23(plugin_test_path, test_tmp_path, call_fstcomp):
    ...
    df = spookipy.MinMaxLevelIndex(src_df, nomvar="TT").compute()

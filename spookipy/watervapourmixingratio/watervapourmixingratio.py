...
class WaterVapourMixingRatio(Plugin):
    ...
    def __init__(
        ...
        if self.ice_water_phase == "water":
            if "temp_phase_switch" in self.explicit_params:
                raise WaterVapourMixingRatioError('Cannot use ice_water_phase="water" with temp_phase_switch\n')
            else:
        ...
        self.plugin_mandatory_dependencies = [
            ...
            {
                "VPPR": {"nomvar": "VPPR", "unit": "Pa"},
                "PX": {"nomvar": "PX", "unit": "Pa"},
            },
        ]

        self.plugin_result_specifications = {"QV": {"nomvar": "QV", "label": "WVMXRT", "unit": "g/kg"}}
    ...
    def prepare_groups(self):
        ...
        mandatory_ice_water_phase_when_using_temp_phase_switch(WaterVapourMixingRatioError, self.explicit_params)

        mandatory_temp_phase_switch_when_using_ice_water_phase_both(
            WaterVapourMixingRatioError, self.explicit_params, self.ice_water_phase, self.rpn
        )
    ...
    def watervapourmixingratio_from_vppr(self, dependencies_df, option):
        ...
        qv_df = create_empty_result(vpprpa_df, self.plugin_result_specifications["QV"], all_rows=True)

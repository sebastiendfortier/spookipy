...
def defines_base_argparser() -> PluginParser:
    ...
    generic_group.add_argument(
        "--help-module", type=str, choices=["generic", "specific"], help="Produces a help for a given module."
    )
    ...
    generic_group.add_argument(
        ...
        "-E",
        action="store_true",
        help="Prints the request this plugin generates when called - can only be used with full configuration because dependencies need to be created. This does not stop the request, thus error may arise if used with a fake request to get this information",
    )
    generic_group.add_argument("--copy_input", "-C", action="store_true", help="Copies the input fields to the output.")
    generic_group.add_argument(
        "--uses",
        action="store_true",
        help="Prints out the plugins that this module uses - can only be used with full configuration because the dependencies need to be created.",
    )
    ...
    generic_group.add_argument(
        ...
        type=str,
        choices=["PYTHON", "CPP"],
        help="Force spooki_run to use the plugin in this language despite the --plugin_language_option.",
    )
...
class Plugin(abc.ABC):
    ...
    def get_dataframes(self):
        ...
        self.no_meta_df = self.df.loc[
            ~self.df.nomvar.isin(["^^", ">>", "^>", "!!", "!!SF", "HY", "P0", "PT"])
    ...
    def final_results(
        ...
        res_df = pd.safe_concat(new_list)

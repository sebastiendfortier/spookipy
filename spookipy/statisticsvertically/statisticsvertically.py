...
STATS : Final[dict] = {'MEAN':'MEAN','STD':'SSTD','PERCENTILES':'C','INTERPERCENTILES':'I','THRESHOLDS':'','NORMTHRESHOLDS':''}
...
class StatisticsVertically(Plugin):
    ...
    def prepare_groups(self):
        ...
        if not (self.nomvar is None):
            ...
            if len(df_list):
                self.no_meta_df = pd.concat(df_list, ignore_index=True)    
    ...
    def validate_params(self):
        ...
        self.validate_options(['MEAN','STD','PERCENTILES','INTERPERCENTILES','THRESHOLDS','NORMTHRESHOLDS'], 'stats')
    ...
    def validate_interpercentiles(self):
        if 'INTERPERCENTILES' in self.stats:
            ...
            if len(self.interpercentiles_lower) != len(self.interpercentiles_upper):
                raise StatisticsVerticallyError(f'interpercentiles_lower and interpercentiles_upper must be of same lenght')
    ...
    def compute(self) -> pd.DataFrame:    
        ...
        for (grid,nomvar,forecast_hour), nomvar_df in self.groups:
            ...
            for stat in self.stats:
                ...
                # print(fstpy.compute(res_df)[fstpy.BASE_COLUMNS].to_string())
    ...
    def parse_config(args: str) -> dict:
        ...
        :rtype: dict
        """
        parser = PluginParser(prog=StatisticsVertically.__name__, parents=[Plugin.base_parser])

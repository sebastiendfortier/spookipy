# -*- coding: utf-8 -*-
from spookipy.humidityutils.humidityutils import calc_water_vapour_mixing_ratio_hu, calc_water_vapour_mixing_ratio_px_vppr, get_temp_phase_switch, validate_humidity_parameters
...
class WaterVapourMixingRatio(Plugin):
    ...
    def validate_input(self):
        ...
        if self.existing_result_df.empty:
            if self.rpn:
                self.dependencies_df = get_plugin_dependencies(self.df,self.plugin_params,self.plugin_mandatory_dependencies_option_1)
            else:
        ...
        for _, current_fhour_group in self.fhour_groups:
            ...
            if self.rpn:
                ...
                for i in qv_df.index:
                    hu = hu_df.at[i,'d']
                    qv_df.at[i,'d'] = calc_water_vapour_mixing_ratio_hu(hu)
                ...
                if self.option==1:
                    ...
                    vppr_df = current_fhour_group.query( 'nomvar=="VPPR"').reset_index(drop=True)
                    ...
                    for i in qv_df.index:
                        vppr = vppr_df.at[i,'d']
                        px = px_df.at[i,'d']
                        qv_df.at[i,'d'] = calc_water_vapour_mixing_ratio_px_vppr(px,vppr)

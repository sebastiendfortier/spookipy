...
class InterpolationHorizontalPoint(Plugin):
    ...
    @initializer
    def __init__(self,df:pd.DataFrame, lat_lon_df:pd.DataFrame, interpolation_type:str='bi-cubic', extrapolation_type:str='maximum', extrapolation_value:float=None):
        self.validate_input()
...
# -73.75,-79.38,-123.18
    ...
    def define_output_grid(self):
        if ('LON' in self.lat_lon_df.nomvar.to_list()) and  ('LAT' in self.lat_lon_df.nomvar.to_list()):
            ...
            self.lon = self.lat_lon_df.loc[self.lat_lon_df.nomvar=="LON"].reset_index(drop=True).iloc[0]['d']
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _,current_group in self.groups:
            ...
            others_df = current_group.loc[~current_group.nomvar.isin(['UU','VV','PT','LAT','LON','>>','^^','^>','!!','HY'])].reset_index(drop=True)
            ...
            vectorial_interpolation(vect_df,results,input_grid,self.output_grid,self.extrapolation_type, self.extrapolation_value, indexes)

            scalar_interpolation(others_df,results,input_grid,self.output_grid,self.extrapolation_type, self.extrapolation_value, indexes)
        ...
        other_res_df = set_new_grid_identifiers(res_df,'Y',len(self.lat),1,self.lat_lon_df.iloc[0]['ip1'],self.lat_lon_df.iloc[0]['ip2'],self.lat_lon_df.iloc[0]['ip3'],0)
...
def scalar_interpolation(df,results,input_grid,output_grid,extrapolation_type,extrapolation_value, indexes=None):
    if df.empty:
        return
    # scalar except PT
...
def vectorial_interpolation(vect_df,results,input_grid,output_grid,extrapolation_type,extrapolation_value,indexes=None):
    if vect_df.empty:
        return

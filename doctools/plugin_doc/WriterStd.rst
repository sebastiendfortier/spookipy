...
`WriterStd <classWriterStd.html>`__
...
`Francais <../../spooki_french_doc/html/pluginWriterStd.html>`__
...
-  Convert each variable to its standard unit designated by the
   `standard variables
   dictionary <http://iweb/~afsypst/spooki/spooki_french_doc/html/stdvar.html>`__,
   with the help of the `UnitConvert <classUnitConvert.html>`__ plug-in,
   unless the "--noUnitConversion" key is activated. During this
   conversion, the second character of TYPVAR is not affected. If the
   original unit is unknown, the conversion cannot be done, the plug-in
   stops by indicating: "The unit is unknown, if you wish to write the
   variable with the original unit, use the &ndash;noUnitConversion
   key."
...
**Usage:**
    ...
    **Results validation:**
    ...
    `Evaluation tree <WriterStd_graph.png>`__

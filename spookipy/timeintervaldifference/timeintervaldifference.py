...
class TimeIntervalDifference(Plugin):
    ...
    def compute(self) -> pd.DataFrame:
        ...
        return self.final_results(df_list, TimeIntervalDifferenceError, copy_input=False, reduce_df=self.reduce_df)
    ...
    def process(self, current_group, b_inf, b_sup, begin_df, end_df):
        ...
        if self.strictly_positive:
            begin_arr = np.where(begin_arr < 0.0, 0.0, begin_arr)
            end_arr = np.where(end_arr < 0.0, 0.0, end_arr)
        ...
        if self.strictly_positive:
            res_df.at[0, "d"] = np.where(res_df.at[0, "d"] < 0.0, 0.0, res_df.at[0, "d"])
        return res_df
    ...
    def check_type_of_params(self):
        ...
        self.forecast_hour_range = validate_list_of_tuples_of_times(
            self.forecast_hour_range, TimeIntervalDifferenceError
        )
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            ...
            required=True,
            type=str,
            help="List of the time intervals between inputs within each time range.",
        )
        parser.add_argument(
            "--rangeForecastHour", required=True, type=str, dest="forecast_hour_range", help="List of time ranges."
        )
        parser.add_argument(
            "--step",
            required=True,
            type=str,
            help="List of the time steps between successive start times within each time range.",
        )
        parser.add_argument(
            "--strictlyPositive",
            action="store_true",
            dest="strictly_positive",
            default=False,
            help="Checks that input and output values are strictly positive.\nThis option also changes the negative values to 0.\nIf said condition is observed a warning will be displayed.",
        )
        ...
        apply_lambda_to_list(
            parsed_arg["nomvar"], lambda a: validate_nomvar(a, "TimeIntervalDifference", TimeIntervalDifferenceError)
        )

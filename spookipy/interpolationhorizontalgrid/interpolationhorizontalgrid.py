...
class InterpolationHorizontalGrid(Plugin):
    ...
    def define_output_grid(self):
        ...
        if self.method == "user":
            if self.grtyp in ["L", "N", "S"]:
                self.ig1, self.ig2, self.ig3, self.ig4 = RmnInterface.grid_definition_to_ig(
                    self.grtyp, self.param1, self.param2, self.param3, self.param4
                )

                infos_grid = fstpy.define_grid(
                    self.grtyp, "", self.ni, self.nj, self.ig1, self.ig2, self.ig3, self.ig4, None, None, None
                )
            ...
            (self.ni, self.nj, self.ig1, self.ig2, self.ig3, self.ig4) = fstpy.set_grid_parameters(field_df)

            # get meta for this fields grid
            grid = field_df.iloc[0]["grid"]

            meta_df = self.df.loc[(self.df.nomvar.isin([">>", "^^", "^>"])) & (self.df.grid == grid)].reset_index(
                drop=True
            )
            ...
            if not meta_df.empty:
                if (">>" in meta_df.nomvar.to_list()) and ("^^" in meta_df.nomvar.to_list()):
                    (self.ni, self.nj, grref, ax, ay, self.ig1, self.ig2, self.ig3, self.ig4) = (
                        fstpy.get_grid_parameters_from_positional_records(meta_df)
                    )

                    infos_grid = fstpy.define_grid(
                        self.grtyp, grref, self.ni, self.nj, self.ig1, self.ig2, self.ig3, self.ig4, ax, ay, None
                    )
                    self.output_grid, *other = infos_grid
            ...
            if (self.output_fields == "interpolated") and (self.method != "user"):
                ...
                self.df = pd.safe_concat([self.df, to_remove]).drop_duplicates(keep=False)
            ...
            if (self.output_fields == "reference") and (self.method != "user"):
                to_remove = self.df.loc[self.df.grid == grid].reset_index(drop=True)
                to_remove = to_remove.loc[(to_remove.nomvar != self.nomvar) & (to_remove.grid == grid)].reset_index(
                    drop=True
                )
                self.df = pd.safe_concat([self.df, to_remove]).drop_duplicates(keep=False)
    ...
    def compute(self) -> pd.DataFrame:
        ...
        for _, current_group in self.groups:
            ...
            others_df = current_group.loc[
                ~current_group.nomvar.isin(["UU", "VV", "PT", ">>", "^^", "^>", "!!", "HY", "!!SF"])
        ...
        if len(results):
            res_df = pd.safe_concat(results)
            res_df = fstpy.set_new_grid_identifiers(
                res_df, self.grtyp, self.ni, self.nj, self.ig1, self.ig2, self.ig3, self.ig4
            )
        ...
        if len(no_mod):
            no_mod_df = pd.safe_concat(no_mod)

        if not no_mod_df.empty:
            res_df = pd.safe_concat([res_df, no_mod_df])

        if not self.all_meta_df.empty:
            res_df = pd.safe_concat([res_df, self.all_meta_df])

        if not self.toctoc_df.empty:
            self.toctoc_df = fstpy.set_new_grid_identifiers_for_toctoc(self.toctoc_df, self.ig1, self.ig2)
            res_df = pd.safe_concat([res_df, self.toctoc_df])

        if not self.hy_df.empty:
            res_df = pd.safe_concat([res_df, self.hy_df])
        ...
        res_no_meta_df = res_df.loc[~res_df.nomvar.isin(["^^", ">>", "^>", "!!", "!!SF", "HY", "PT"])].reset_index(
            drop=True
        )

        return self.final_results(
            [res_no_meta_df], InterpolationHorizontalGridError, copy_input=False, reduce_df=self.reduce_df
        )
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            ...
            type=str,
            dest="method",
            choices=["FIELD_DEFINED", "USER_DEFINED"],
            required=True,
            help="Manner in how the target grid is defined",
        )
        parser.add_argument(
            "--fieldName",
            type=str,
            dest="nomvar",
            help="Name of the field on the grid to which interpolate. \nMandatory if '--outputGridDefinitionMethod FIELD_DEFINED' is used.",
        )
        parser.add_argument(
            "--gridType",
            type=str,
            choices=["TYPE_A", "TYPE_B", "TYPE_G", "TYPE_L", "TYPE_N", "TYPE_S"],
            dest="grtyp",
            help="Type of target grid (see the reference: Grid types supported by RPN Standard Files). \nMandatory if '--outputGridDefinitionMethod USER_DEFINED' is used.",
        )
        parser.add_argument(
            "--xyDimensions",
            type=str,
            help="X and Y dimensions of the target grid. \nMandatory if '--outputGridDefinitionMethod USER_DEFINED' is used.",
        )
        parser.add_argument(
            "--gridProjectionParameters",
            "-p",
            type=str,
            help="Projection parameters 1 to 4 that define the target grid  (see the reference: Grid types supported by RPN Standard Files). \nMandatory if '--outputGridDefinitionMethod USER_DEFINED' is used.",
        )
        parser.add_argument(
            "--interpolationType",
            type=str,
            default="BI-CUBIC",
            choices=["NEAREST", "BI-LINEAR", "BI-CUBIC"],
            dest="interpolation_type",
            help="Type of interpolation.",
        )
        parser.add_argument(
            "--extrapolationType", type=str, default="MAXIMUM", dest="extrapolation_type", help="Type of extrapolation."
        )
        parser.add_argument(
            "--outputField",
            type=str,
            default="INCLUDE_ALL_FIELDS",
            choices=["INTERPOLATED_FIELD_ONLY", "INCLUDE_REFERENCE_FIELD", "INCLUDE_ALL_FIELDS"],
            dest="output_fields",
            help="Choice of output: include interpolated fields only or include reference field with interpolated fields or include all fields.",
        )
        ...
        if parsed_arg["method"] == "field" and parsed_arg["nomvar"] is None:
            raise InterpolationHorizontalGridError(
                "--fieldName is mandatory if '--outputGridDefinitionMethod FIELD_DEFINED' is used."
            )
        elif parsed_arg["method"] == "user":
            if parsed_arg["grtyp"] is None:
                raise InterpolationHorizontalGridError(
                    "--gridType is mandatory if '--outputGridDefinitionMethod USER_DEFINED' is used."
                )
            if parsed_arg["xyDimensions"] is None:
                raise InterpolationHorizontalGridError(
                    "--xyDimensions is mandatory if '--outputGridDefinitionMethod USER_DEFINED' is used."
                )
            if parsed_arg["gridProjectionParameters"] is None:
                raise InterpolationHorizontalGridError(
                    "--gridProjectionParameters is mandatory if '--outputGridDefinitionMethod USER_DEFINED' is used."
                )
        ...
        if parsed_arg["gridProjectionParameters"] is not None:
            ...
            if (
                parsed_arg["param1"] < 0
                or parsed_arg["param2"] < 0
                or parsed_arg["param3"] < 0
                or parsed_arg["param4"] < 0
            ):
        ...
        output_fields = {
            "INTERPOLATED_FIELD_ONLY": "interpolated",
            "INCLUDE_REFERENCE_FIELD": "reference",
            "INCLUDE_ALL_FIELDS": "all",
        }
...
def set_extrapolation_type_options(extrapolation_type: str, extrapolation_value: Optional[float]) -> None:
    ...
    if extrapolation_type == "value":
        ...
        RmnInterface.set_interpolation_values("EXTRAP_VALUE", extrapolation_value)
        RmnInterface.set_interpolation_options("EXTRAP_DEGREE", "VALUE")
    else:
        # print( self.extrapolation_type.upper())
        RmnInterface.set_interpolation_options("EXTRAP_DEGREE", extrapolation_type.upper())
...
def scalar_interpolation_pt(df: pd.DataFrame, results: List[pd.DataFrame], input_grid: int, output_grid: int) -> None:
    ...
    extrap_degree = RmnInterface.get_interpolation_options("EXTRAP_DEGREE", vtype=str)
    RmnInterface.set_interpolation_options("EXTRAP_DEGREE", "NEAREST")

    scalar_interpolation(df, results, input_grid, output_grid, None, None, None)

    RmnInterface.set_interpolation_options("EXTRAP_DEGREE", extrap_degree)

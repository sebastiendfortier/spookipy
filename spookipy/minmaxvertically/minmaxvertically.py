...
from ..utils import get_from_dataframe, initializer, reshape_arrays, validate_nomvar
...
class MinMaxVertically(Plugin):
    ...
    def validate_params_and_input(self):
        if not (self.nomvar_min is None):
            validate_nomvar(self.nomvar_min, "MinMaxVertically", MinMaxVerticallyError)

        if not (self.nomvar_max is None):
            validate_nomvar(self.nomvar_max, "MinMaxVertically", MinMaxVerticallyError)
    ...
    def compute(self) -> pd.DataFrame:
        ...
        return self.final_results(df_list, MinMaxVerticallyError, copy_input=self.copy_input, reduce_df=self.reduce_df)
    ...
    def parse_config(args: str) -> dict:
        ...
        parser.add_argument(
            ...
            default="MIN",
            dest="nomvar_min",
            help="Option to change the name of output field MIN",
        )
        parser.add_argument(
            "--outputFieldName2",
            type=str,
            default="MAX",
            dest="nomvar_max",
            help="Option to change the name of output field MAX",
        )
